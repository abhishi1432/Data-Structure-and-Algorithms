...
int index=0;
TreeNode* buildTree_helper(vector<int>& preorder,int start,int end,map<int,int>&hash_map) {
	 if(start > end){
	 	return NULL;
	 }   
	 int value = preorder[index++];
	 TreeNode* root = createNode(value);
	 root->left = buildTree_helper(preorder,start,hash_map[value]-1,hash_map);
	 root->right = buildTree_helper(preorder,hash_map[value]+1,end,hash_map);
	 return root;
}
TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
	...
	int index=0;
	return buildTree_helper(preorder,0,preorder.size()-1,mp);	
}
...
void solve() {
	...
	cout<<ans->val<<nline;
	cout<<ans->left->val<<nline;
	cout<<ans->right->val<<nline;
