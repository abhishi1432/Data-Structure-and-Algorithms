...
void Print_All_Subsequence(vector<int> &arr , vector<int>&ds, int ind, int n){
	...
	*/
	if(ind == n){   //base case when all the elements are traversed
		cout<<"{ ";
		for(auto it:ds)
			cout<<it<<" ";
		if(ds.size()==0)
	...
	ds.push_back(arr[ind]);  //when we take the element
	Print_All_Subsequence(arr, ds, ind+1, n);
	ds.pop_back();  //when we we do not take the element
	Print_All_Subsequence(arr, ds, ind+1, n);
}

int print_Number_Of_subsequence_with_Sum_K(vector<int> &arr , int k, int sum, int ind, int n){
	/*
		Time complexity of this solution is O(2^n)
		Space complexity  is O(n)
		Note : Merging the print all subsequne with this code can be used to print subsequnces with sum k.
	*/
	int ans=0;
	if(ind == n){
		if(sum == k)
	...
	sum+=arr[ind];
	ans+= print_Number_Of_subsequence_with_Sum_K(arr,k,sum,ind+1,n);
	sum-=arr[ind];
	ans+=print_Number_Of_subsequence_with_Sum_K(arr,k,sum,ind+1,n);
	return ans;
...
void solve() {
	...
	cout<<print_Number_Of_subsequence_with_Sum_K(arr,k,0,0,arr.size())<<nline;
