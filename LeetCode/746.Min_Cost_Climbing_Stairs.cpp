...
/*=================================================================*/
int minCostClimbingStairs_naive(vector<int>& cost,int n){
	if(n>=cost.size())
		return 0; 
	return cost[n] + min(minCostClimbingStairs_naive(cost,n+1),minCostClimbingStairs_naive(cost,n+2));
}
int minCostClimbingStairs_memo(vector<int>& cost, int n, vector<int>&memo){
	if(n>=cost.size())
		return 0; 
	if(memo[n]!=-1)
		return memo[n];
	return memo[n] = cost[n] + min(minCostClimbingStairs_memo(cost,n+1,memo),minCostClimbingStairs_memo(cost,n+2,memo));
}
...
int minCostClimbingStairs(vector<int>& cost) {
	...
	// return min(minCostClimbingStairs_memo(cost,0,memo), minCostClimbingStairs_memo(cost,1,memo));     
	return minCostClimbingStairs_dp(cost);
}
