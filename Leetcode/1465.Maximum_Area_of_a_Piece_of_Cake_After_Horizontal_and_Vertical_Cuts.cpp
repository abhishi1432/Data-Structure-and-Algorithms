...
int maxArea(int h, int w, vector<int>& horizontalCuts, vector<int>& verticalCuts) {
	...
	debug(max1);
	if(len1!=0){
		if(h-horizontalCuts[len1-1]>max1)
			max1=h- horizontalCuts[len1-1];
	}
	else
		max1 = h;
	int max2=0;
	prev=0;
	for(auto elm: verticalCuts){
		if(elm-prev >max2)
			max2=elm-prev;
		prev=elm;
	}
	if(len2!=0){
		if(w- verticalCuts[len2-1]>max2)
			max2=w- verticalCuts[len2-1];
	}
	else
		max2=w;
	debug(max2);
...
void solve() {
	...
	input_vec(horizontalCuts,n1);
	input_vec(verticalCuts,n2);
	cout<<maxArea(h,w,horizontalCuts,verticalCuts)<<nline;
