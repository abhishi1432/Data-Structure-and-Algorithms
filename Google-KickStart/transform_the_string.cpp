...
void solve() {
		...
		while(test<=t){
			...
			int len_s=sz(s);
			int len_f=sz(f);
			vector<int> alphabet_count_f(27,0);
			for(int i=0;i<len_f;i++){
				alphabet_count_f[int(f[i])-96]+=1;
			}
			vector<int> alphabet_count_s(27,0);
			for(int i=0;i<len_s;i++){
				alphabet_count_s[int(s[i])-96]+=1;
			}
			debug(alphabet_count_s)
			debug(alphabet_count_f)
			int ans=0;
			for(int i=0;i<=26;i++){
				if (alphabet_count_s[i]!=0){
				int min_i=50;
				for(int j=1;j<=26;j++){
					if(alphabet_count_f[j]!=0){
						int temp=min(abs((char)(96+i)-(char)(96+j)),abs(26-abs((char)(96+i)-(char)(96+j))));
						min_i=min(min_i,temp);
					}
				}
				if(min_i!=50){
					debug(min_i)
					ans=ans+min_i*alphabet_count_s[i];
				}
			}
		}
			google(test);
