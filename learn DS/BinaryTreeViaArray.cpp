...
//use Ctrl+shift+[ or ] to fold and unfold the code
...
int BinaryTree::getLeft(int index){return 2*index+1;
}

int BinaryTree::getRight(int index){return 2*index+2;
}

int BinaryTree::getParent(int index){return (index-1)/2;
}
...
void solve() {
	...
	cout<<(bt.isEmpty() ? "IsEmpty? True" : "IsEmpty? False") <<nline;
	...
	cout<<"Key found at "<<bt.searchKey(4)<<nline;
	
	cout<<"CurrentSize is "<<bt.currentSize<<nline;
