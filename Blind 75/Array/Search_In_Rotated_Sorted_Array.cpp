...
int binarySearchh(vector<int> arr, int l, int r, int x)
{
    if (r >= l) {
        int mid = l + (r - l) / 2;
        if (arr[mid] == x)
...
int findMin_2(vector<int> nums,int k){
 	/*
		Time complexity is O(logn)...using modified binary search first we find the smallest element which will be our pivot 
		To understand the implementation of this take example :
		Ex:  1 3 5 7  
			 7 1 3 5
			 5 7 1 3
			 3 5 7 1
		...
		1- nums[low]<nums[mid]<nums[high] -> answer is nums[low]
		2- nums[low]>nums[mid]<nums[high] -> Go to 1st half and repeat process
		3- nums[low]<nums[mid]>nums[high] -> Go to 2nd half and repeat process
		If at last only 2 elements left then return the smaller element
 	*/
 	int n=nums.size();
 	...
 	/*
	Now as we have found our pivot which is contained in variable flag
 	*/
 	if(flag == 0){
 		low=0;
 		high=n-1;
	 ...
	 return binarySearchh(nums,low,high,k);
 }
