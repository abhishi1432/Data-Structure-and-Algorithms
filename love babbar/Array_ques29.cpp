...
void find3numbers_usingHashMap(vector<int> arr,int n,int X){
	/*  Tried myself and but did not get the AC
	...
	for(int i=0;i<n;i++){
		int temp=X-arr[i];
		for(int j=0;j<n;j++){
			result=result+mp1[temp-arr[j]];
			if(temp-arr[j]==arr[j])
				result--;   //we decrese 1 as a arr[i] is already counted
			if(arr[j]==arr[i])
				result--;
			if(mp1[temp-arr[j]]>0)
				cout<<arr[i]<<" "<<arr[j]<<" "<<temp-arr[j]<<nline;
		}
	}
	cout<<result/6<<nline;
	*/

    /*Here now I did some modification and then got the AC */
	unordered_map<int, int> mp1;
	int result=0;
	for(int i=0;i<n;i++)
		mp1[arr[i]]+=1;
...
void find3numbers_usingSorting_and_TwoPointer(vector<int> A,int n,int X){
	sort(all(A));
	int result=0;
	for(int i=0;i<n;i++){
...
void solve() {
	...
	vector<int> arr;
	input_vec(arr,n);
	// find3numbers_usingHashMap(arr,n,X);  //Time complexity O(n^2) and Space complexity is O(n)
	find3numbers_usingSorting_and_TwoPointer(arr,n,X);   //Time Complexity is O(n^2) and space complexity is O(1)
}
