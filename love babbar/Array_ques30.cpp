...
void trappingWater_method1(int n,vector<int> arr){
	/*
...
void trappingWater_method2(int n,vector<int> arr){
	/*
	Time complexity of this solution is O(n)
	Space complexity is O(n+n)
	*/
	vector<int> max_left(n,-1),max_right(n,-1);
	int maxNo=INT_MIN;
	for(int i=0;i<n;i++){
	...
	int res=0;
	for(int i=0;i<n;i++){
		res=res+ min(max_left[i],max_right[i]) - arr[i];
	}
	cout<<res<<nline;
}
void trappingWater_method3(int n,vector<int> arr){
	...
	While l <= r , iterate the array .
	We have two possible conditions
	...
	*/
	int maxLeft=-1,maxRight=-1,low=0,high=n-1,res=0;
	while(low<=high){
		if(arr[low]<arr[high]){
			if(arr[low]>maxLeft)
				maxLeft=arr[low];
...
void trappingWater_method4(int n,vector<int> arr){
	stack <int> st;
...
void solve() {
	...
	vector<int> arr;
	input_vec(arr,n);  
	trappingWater_method1(n,arr);   //Naive approach
	trappingWater_method2(n,arr);	//using two additional arrays to store left and right max for all indexs i
	trappingWater_method3(n,arr);  //This is a good one using two pointer approach. Try to understand this
	trappingWater_method4(n,arr)   //Using stack
} 
