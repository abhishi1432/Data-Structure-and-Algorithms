...
Node* reverseLinkedList_Recursive(Node* head,Node** new_head){
	if(head  == NULL){
        *new_head = head;
        return head;
    }
	...
	Node* temp = reverseLinkedList_Recursive(head->next,new_head);
	temp->next = head;
	temp->next->next = NULL;
	return head;
}
...
void solve() {
	...
	Node* new_head;
	reverseLinkedList_Recursive(head2,&new_head);
	printLinkedList(&new_head);
	reverseLinkedList_Iterative(&new_head);
	printLinkedList(&new_head);
}
