...
/*=================================================================*/
void Brute_force(int n,int k, vector<int> arr){
	int result=0;
	for(int i=0;i<n;i++){
		for(int j=i+1;j<n;j++){
			if(arr[i]+arr[j]==k)
...
void optimized_soln(int n,int k,vector<int> arr){

	int result=0;
	unordered_map<int,int> mp; //took unordered map as its complexity is O(1) therefore overall O(n)
	// map<int,int> mp;        //for ordered map complexity is O(log n) therefor overall O(nlogn)
	for(int i=0;i<n;i++){
		mp[arr[i]]++;
	}
	for(int i=0;i<n;i++){
		result=result+mp[k-arr[i]];
		if(k-arr[i]==arr[i])
