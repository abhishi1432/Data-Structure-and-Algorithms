...
void solution_usingHash(vector<int> arr,int n){
	/*
	Time complexity of this function is O(n*log(n)),
	can be optimised to O(n) by using ordered map with complexity O(1) instead of O(logn)
	...
	*/
	...
	for(int i=0;i<n;i++){
		sum+=arr[i];
		if(mp[sum].first==1)
			vec.push_back({mp[sum].second+1,i});
		if(sum==0)
			vec.push_back({0,i});
		mp[sum]={1,i};
	}
