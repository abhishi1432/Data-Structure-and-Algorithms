...
int Kth_usingSet(vector<int> arr,int l,int r,int k){

	/*As set in c++ STL uses Binary search tree with complexities(searching, sorting, deleting) 
	as  O(log(n)) in best and O(n) in worst case */
	/*Applicable only when all elements are different*/
	set<int> ss(all(arr));
	debug(ss)
	set<int>::iterator itr=ss.begin(); // auto itr=ss.begin()
	advance(itr,k-1);  // itr point to kth element in set
	return (*itr);
}

int Kth_usingMap(vector<int> arr,int l,int r,int k){
	/*Similar to set, Map also stores in sorted manner so we can easily get the kth element*/
	map<int,int> mp;
	for(auto i:arr)
		mp[i]+=1;
	debug(mp)
...
int Kth_method2(vector<int> arr,int l,int r,int k){
	return 0;
}

void solve() {
	vector<int> arr={1,6,3,8,3,9,5,12};
	int k;
	cin>>k;
	int ans=Kth_usingSet(arr,0,sz(arr)-1,k); 
	cout<<ans<<nline;
	ans=Kth_usingMap(arr,0,sz(arr)-1,k);
	cout<<ans<<nline;
	sort(all(arr));
	debug(arr);
}
