...
int maxDiameter_Optimized(TreeNode* root,int &ans) {
	...
	*/
    ...
    //here basically we are taking 0 if lh is -ve or rh is -ve.
    ans = max(ans, (leftMax < 0 ? 0 : leftMax) + (rightMax < 0 ? 0 : rightMax) + root->data);
    return root->data + (max(leftMax,rightMax)< 0 ? 0 : max(leftMax,rightMax));
}
...
void solve() {
	...
	*/
	...
	// root2->right->right->right = createNode(12);
	// root2->right->right->right->right = createNode(23);
	cout<<maxPathSum(root2)<<nline;
